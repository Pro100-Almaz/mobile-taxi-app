...
async def connect_websocket(websocket: WebSocket):
    ...
    try:
        ...
        await websocket.send_json({"socket_id": socket_id, "msg": "connected"})

        while True:
            # Receive JSON from browser
            data = await websocket.receive_json()
            print(f"Received from {socket_id}: {data}")

            # Echo it back as JSON
            await websocket.send_json({"echo_from_server": data})
    except WebSocketDisconnect:
